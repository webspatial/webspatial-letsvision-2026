 ...
 * that slide, or until the next element with a class of "comment".
 * Such comments are hidden in slide mode.
 *
 ...
 *
 * More documentation:
 *   - https://www.w3.org/Talks/Tools/b6plus/Overview.html
 *     How to write and present slide decks and how to use features of
 *     the ‘simple.css’ style sheet.
 *   - https://www.w3.org/Talks/Tools/b6plus/writing-style-sheets.html
 *     Everything for writing (advanced) style sheets for b6+.
 *
 * Usage:
 *
 ...
 *   and left arrow or up arrow to go back.
 *
 * - Press Page-Down or Page-Up to skip any incrementally displayed
 *   elements (built-up slides) and go directly to the next, resp.
 *   previous slide.
 *
 * On touch screens, a tap with three fingers toggles slide mode, a
 * swipe right goes back, and swipe left advances.
 *
 ...
 *
 * TODO: Why does Chrome (but not Vivaldi) lose keyboard focus when
 * leaving fullscreen? And is there a workaround?
 *
 * TODO: Maybe make the Play button into a combined button/dropdown,
 * that allows to choose what the button does: open in full screen,
 * open in the current window, and maybe also open in 2nd window (and
 * then the 2nd window button is not needed anymore).
 *
 ...
 * Modified: Nov 2025 (key "N" to speak speaker notes)
 * Modified: Dec 2025 (No role=application, ToC shows current, use bubbled keys)
 *
...
const translations = {
  ...
  "<kbd>N</kbd>": {},
  "(screen reader) speak notes/next slide/current element": {
    de: "(Screenreader) Notizen/nächste Folie/aktuelles Element vorlesen",
    fr: "(lecteur d'écran) notes de présentation/diapo suivante/élément actuel",
    nl: "(schermlezer) notities/volgende dia/huidig element uitspreken"},
  ...
  "Current content (live region)": {
    de: "Aktueller Inhalt (Live-Bereich)",
    fr: "Contenu actuel (zone dynamique)",
    nl: "Huidige inhoud (live region)"},
  "Current item: ": {
    de: "Aktuelles Element: ",
    fr: "Objet actuel : ",
    nl: "Huidig item: "},
};
...
/* Things to speak by screen readers. */
const TOGGLE = Symbol("toggle");
const SLIDE = Symbol("slide");
const COMMENT = Symbol("comment");
const NEXT = Symbol("next");
const INCREMENTAL = Symbol("incremental");
const EXIT = Symbol("exit");

/* Units for duration and data-timing attributes. */
const SECONDS = 1000;
...
var slideCopy = [];		// Elements that can be put in liveregion
var commentCopy = [];		// Elements that can be put in liveregion
var lastSpoken = SLIDE;		// For the "N" key
...
var tocItems = [];		// The LI items in the table of contents
...
var useApplication = false;	// Whether to set role=application in slide mode
...
/* setStyle -- set/unset multiple properties in an element's style attribute */
function setStyle(elt, ...decls)
{
  var i, j, r = "", p;

  // The decls alternate property names and values. To remove a
  // property, pass an empty string or null as its value.

  // This function allows setting multiple properties at once and
  // should thus avoid some reflows compared to calling
  // elt.style.SetProperty() several times.

  console.assert(decls.length % 2 == 0);

  // Copy to r all the properties from elt.style that do not occur in decls.
  for (j = 0; j < elt.style.length; j++) {
    p = elt.style[j];
    for (i = 0; i < decls.length && p !== decls[i]; i += 2);
    if (i >= decls.length)
      r = r.concat(p, ":", elt.style.getPropertyValue(p),
	elt.style.getPropertyPriority(p), ";");
  }

  // Append to r all the properties from decls.
  for (i = 0; i < decls.length; i += 2)
    r = r.concat(decls[i], ":", decls[i+1], ";");

  // Replace elt.style with the result.
  elt.style.cssText = r;
  return elt;
}
...
/* cloneNodeClean -- deep clone a node, without any attributes except lang */
function cloneNodeClean(elt)
{
  var clone, a, h;

  // The clone is only used for speaking by a screen reader, for which
  // it may be put in a live region. Nothing in the clone should ever
  // get the focus(), so we set tabindex=-1. (It seems Safari
  // otherwise may set the focus to interactive elements even though
  // they are invisible.)
  clone = elt.cloneNode(false);
  if (elt.nodeType === Node.ELEMENT_NODE) {
    for (a of elt.attributes)
      if (! a.name.match(/^(lang|aria-label|aria-hidden)$/))
	clone.removeAttribute(a.name);
    clone.tabIndex = -1;
  }
  for (h = elt.firstChild; h; h = h.nextSibling)
    clone.appendChild(cloneNodeClean(h));		// Recursive
  return clone;
}
...
function initClocks()
{
  ...
  for (const c of document.body.classList) {
    if ((t = c.match(/^duration=(.*)/))) duration =timeToMillisec(t[1],MINUTES);
    if ((t = c.match(/^warn=(.*)/))) warnTime = timeToMillisec(t[1], MINUTES);
  }
...
{
  return elt &&
    (elt.b6slidenum !== undefined ||	// shortcut: we already numbered it
...
function updateProgress()
{
  ...
  /* Set a custom variable on BODY for use by style rules. This one is
     0 for the first slide */
  if (numslides > 1)
    document.body.style.setProperty('--progress0',
      (curslide.b6slidenum - 1) / (numslides - 1));
  else
    document.body.style.setProperty('--progress0', 1);
}
...
{
  /* Unhide all elements that are progress bars. */
  if (interactive) {
    for (const e of progressElts)
      unhideElement(e);
  }

  /* Unhide all elements that should contain the current slide number. */
...
function numberSlides()
{
  ...
  // Find the elements that need updating later.
...
function scaleImagesIfNeeded(slide)
{
  ...
  // Do the repeated layouts in the next animation frame, to avoid
  // that the slide is painted in the middle of the loop.
  requestAnimationFrame(() => {
    ...
    setStyle(slide, "height", "auto", "max-height", "none");
    ...
    // Restore the element's style sttribute. (The saved values may be null.)
    setStyle(slide, "height", saveHeight, "max-height", saveMaxHeight);
  });
}
...
{
  var height, f, f1, f2, wrapper, saveHeight, saveMaxHeight;
...
/* hideElement -- make an element invisible */
function hideElement(elt)
{
  if (elt.b6savedstyle === undefined)
    elt.b6savedstyle = elt.style.cssText;	// Remember properties
  setStyle(elt, "position", "absolute", "visibility", "hidden", "top", "0",
    "left", "0", "width", "0", "height", "0");
  return elt;
}


/* unhideElement -- make a previously hidden element visible again */
function unhideElement(elt)
{
  // If b6savedstyle is undefined, the elt has not yet been hidden.
  if (typeof(elt.b6savedstyle) === "string")
    elt.style.cssText = elt.b6savedstyle;
  return elt;
}


/* hideSlide -- make a slide invisible, return last element made invisible */
function hideSlide(elt)
{
  var h;

  // Unlike hideElement(), hideSlide() only sets 'visibility: hidden'
  // and 'position: absolute'. We assume that the slide is not bigger
  // than other slides and thus its size doesn't increase the size of
  // the BODY and need not be set to 0. This allows the style sheet to
  // offer transitions, because it only needs to reset the visibility
  // and doesn't need to worry about the size.
  if (elt.b6savedstyle === undefined) elt.b6savedstyle = elt.style.cssText;
  setStyle(elt, "visibility", "hidden", "position", "absolute", "top", "0");
  if (! elt.classList.contains("slide"))
    while ((h = elt.nextElementSibling) && ! isStartOfSlide(h) &&
	! h.classList.contains("comment")) {
      if (h !== liveregion) {
	if (h.b6savedstyle === undefined) h.b6savedstyle = h.style.cssText;
	setStyle(h, "visibility", "hidden", "position", "absolute", "top", "0");
      }
      elt = h;
    }
  return elt;
...
/* unhideSlide -- make a (previously hidden) slide visible */
function unhideSlide(elt)
{
  var h;

  console.assert(typeof(elt.b6savedstyle) === "string");
  console.assert(isStartOfSlide(elt));

  elt.style.cssText = elt.b6savedstyle;
  if (! elt.classList.contains("slide"))
    while ((h = elt.nextElementSibling) && ! isStartOfSlide(h) &&
	! h.classList.contains("comment")) {
      if (h !== liveregion) h.style.cssText = h.b6savedstyle;
      elt = h;
    }
  return elt;
}
...
/* speakIt -- speak a text and an array of nodes */
function speakIt(text, elts)
{
  if (document.ariaNotify)
    // TODO: Handle the case that the text is in a different language
    // from the document or even in multiple languages.
    document.ariaNotify(text +
	elts.map(e => cloneNodeClean(e))
	    .map(e => e.innerText ?? e.textContent)
	    .join(""));
  else {
    // TODO: Make sure nothing in the liveregion can get keyboard
    // focus on tabbing.
    liveregion.setAttribute("aria-busy", "true");
    liveregion.textContent = text;
    liveregion.append(...elts.map(e => cloneNodeClean(e)));
    liveregion.setAttribute("aria-busy", "false");
  }
}


/* speak -- set the live region to the slide content or the speaker notes */
function speak(what)
{
  var h, next, elts = [];

  // NEXT is not expected as argument.
  console.assert(what === SLIDE || what === COMMENT || what === TOGGLE ||
      what === INCREMENTAL || what === EXIT);
  // EXIT is not part of the cycle indicated by lastSpoken.
  console.assert(lastSpoken === SLIDE || lastSpoken === COMMENT ||
      lastSpoken == NEXT || lastSpoken === INCREMENTAL);

  // If TOGGLE, use lastSpoken to cycle to the next item. Otherwise
  // reset lastSpoken to the start of the cycle, i.e., SLIDE.
  if (what === TOGGLE) {
    if (lastSpoken === SLIDE) what = lastSpoken = COMMENT;
    else if (lastSpoken === COMMENT) what = lastSpoken = NEXT;
    else if (lastSpoken === NEXT)
      what = lastSpoken = 0 <= incrementals.cur &&
      incrementals.cur < incrementals.length ? INCREMENTAL : SLIDE;
    else what = lastSpoken = SLIDE;	// lastSpoken === INCREMENTAL
  } else
    lastSpoken = SLIDE;			// Reset toggle

  switch (what) {
  case SLIDE:
    elts.push(curslide);
    if (! curslide.classList.contains("slide"))
      for (h = curslide.nextElementSibling;
	h && ! isStartOfSlide(h) && ! h.classList.contains("comment");
	h = h.nextElementSibling)
	if (h !== liveregion) elts.push(h);
    speakIt(_("Slide") + " " + curslide.b6slidenum + ": ", elts);
    break;
  case COMMENT:
    // Speaker notes are modeled as sibling `.comment` blocks after a slide
    // in this repo, so collect them for both explicit `section.slide`
    // slides and legacy H1-delimited slides.
    for (h = curslide.nextElementSibling;
	h && ! isStartOfSlide(h);
	h = h.nextElementSibling)
      if (h.classList.contains("comment")) elts.push(h);
    speakIt(elts.length ? _("Notes: ") : _("No notes"), elts);
    break;
  case INCREMENTAL:
    console.assert(0 <= incrementals.cur &&
	incrementals.cur < incrementals.length);
    speakIt(_("Current item: "), [incrementals[incrementals.cur]]);
    break;
  case NEXT:			// Speak next slide
    next = curslide.nextElementSibling;
    while (next && ! isStartOfSlide(next)) next = next.nextElementSibling;
    if (next) {
      elts.push(next);
      if (! next.classList.contains("slide"))
	for (h = next.nextElementSibling;
	  h && ! isStartOfSlide(h) && ! h.classList.contains("comment");
	  h = h.nextElementSibling)
	  if (h !== liveregion) elts.push(h);
    }
    speakIt(next ? _("Next slide: ") : _("No next slide"), elts);
    break;
  case EXIT:
    speakIt("", savedContent);
    break;
  }
}
...
function displaySlide()
{
  ...
  unhideSlide(curslide);
  curslide.classList.add("active");		// Compatibility with Shower
  scaleImagesIfNeeded(curslide);
  scaleFontIfNeeded(curslide);
  scaleBody();
...
/* undisplaySlide -- make the current slide invisible */
function undisplaySlide()
{
  var h;
...
/* toggleFullscreen -- toggle fullscreen or turn it on ("on") or off ("off") */
async function toggleFullscreen(onoff)
{
  ...
  if (onoff !== "on" && document.fullscreenElement)
	...
	(x = (await window.getScreenDetails()).screens.filter(
	  (h) => !h.isInternal)) && x.length == 1 ? x[0] : screen;
  ...
  // Chrome seems to lose the focus when exiting fullscreen. Try to set it.
  document.body.focus();
}
...
function createHelpText()
{
  ...
  iframe.srcdoc =
    "<!DOCTYPE html>" +
	  ...
	  "<tr><td>" + _("<kbd>N</kbd>") +
	  "<td>" + _("(screen reader) speak notes/next slide/current element")) +
      (!syncURL ? "" :
	  "<tr><td>" + _("<kbd>S</kbd>") +
  ...
  button.textContent = "\u274C\uFE0E"; // Cross mark
...
function createTOCText()
{
  ...
  for (const h of document.body.children)
    if (isStartOfSlide(h)) {
      ...
      tocItems[h.b6slidenum] = document.createElement("li");
      tocItems[h.b6slidenum].innerHTML =
	'<a href="' + i + '">' + (t ?? '#' + h.b6slidenum) + '</a>';
      ol.append(tocItems[h.b6slidenum]);
    }
  ...
  button.textContent = "\u274C\uFE0E"; // Cross mark
  button.addEventListener('click',
    ev => {toctext.close(); toctext.remove(); ev.stopPropagation()});
  ...
  for (const e of toctext.getElementsByTagName('A'))
    ...
    e.addEventListener('click', ev => {toctext.close(); toctext.remove()});

  /* Do not let the system close the dialog on a press of the Escape
   * key, because we want to capture the key ourselves. That way we
   * can stop it from propagating and being handled by keyDown() on
   * the body. ("closedBy" only works in a few very recent browsers so
   * far: Firefox 141, July 2025, and Chrome 134, March 2025.) */
  toctext.closedBy = "none";
  /* Unfortunately, the Escape key exits fullscreen mode. We can only
   * intercept it in Chrome, see lock() in toggleMode(). */
  toctext.addEventListener('keydown', ev => {
    //console.log("toctext", ev.key, ev.target);
    if (ev.key === 'c' || ev.key === 'Escape') {
      toctext.close(); toctext.remove();
      ev.preventDefault(); ev.stopPropagation()}});

  /* When the ToC is closed, clear any "aria-current" attribute. */
  toctext.addEventListener('close',
    // eslint-disable-next-line no-unused-vars
    ev => tocItems[curslide.b6slidenum].removeAttribute("aria-current"));

  /* Define b6savedstyle in case unhideElement() is called on toctext. */
  toctext.b6savedstyle = toctext.style.cssText;
}
...
function tableOfContents()
{
  ...
  tocItems[curslide.b6slidenum].setAttribute("aria-current", "true");
...
function openSecondWindow()
{
  ...
  if (location.protocol === 'about:') {
    warningBanner('This is a temporary document. It cannot be opened in another window.');
    return;
  }
...
function warningBanner(...content)
{
  ...
  if (slidemode) setStyle(banner, "top", "auto", "bottom", "0");
  else setStyle(banner, "top", "0", "bottom", "auto");
...
function errorSyncMode(ev)	// eslint-disable-line no-unused-vars
{
...
function toggleAnnotate()
{
  if (!canvas) {
    ...
    setStyle(canvas, "position", "fixed", "top", "0", "left", "0",
      "visibility", "hidden");
...
/* keyDown -- handle key presses anywhere on the document */
function keyDown(event)
{
  // console.log(`keyDown ${event.key} target=${event.target} currentTarget=${event.currentTarget} eventPhase=${event.eventPhase}`);

  // We handle key presses also if they bubble up from other elements,
  // unless we believe the elements handled them. (Elements may handle
  // keys that we're not interested in, such as the Enter key, but as
  // we'll ignore those anyway, we don't need to check.)
  if (event.target.tagName.match(/^(?:SELECT|VIDEO|AUDIO|TEXTAREA)$/) ||
      (event.target.tagName === "INPUT" &&
       ! event.target.type.match(/^(?:checkbox|radio|submit|reset|image)$/)) ||
      (event.key === ' ' &&
       event.target.tagName.match(/^(INPUT|BUTTON|SUMMARY)$/)))
    return;
  ...
  case 'w':
    ...
    break;
  case 'n':
    // if (syncmode) warnSyncMode() else
    if (slidemode) speak(TOGGLE);
    else if (secondwindow?.closed !== false) return // No 2nd window
    else secondwindow.postMessage({event: "keydown", v: event.key});
    break;
  default:
...
function scaleBody()
{
  ...
  requestAnimationFrame(() => {
    if (!slidemode) return;	// In case the slide show just ended
    // console.log(`offsetWidth=${document.body.offsetWidth} offsetHeight=${document.body.offsetHeight} clientWidth=${document.body.clientWidth} clientHeight=${document.body.clientHeight} scrollWidth=${document.body.scrollWidth} scrollHeight=${document.body.scrollHeight}`);
    save = document.body.style.contain;
    setStyle(document.body, "position", "relative", "top", "0", "left", "0",
      "contain", "layout");
    // The 'contain: layout ensures that the size includes children's margins
    w = document.body.offsetWidth;
    h = document.body.offsetHeight;
    w2 = window.visualViewport.width;
    h2 = window.visualViewport.height;
    scale = w > 1 && h > 1 ? Math.min(w2/w, h2/h) : 1;
    // console.log(`scaleBody ${w}x${h} -> ${w2}x${h2} -> ${scale}`);
    setStyle(document.body, "transform", `scale(${scale})`,
      "margin", `${(h2 - h)/2}px ${(w2 - w)/2}px`, "contain", `${save}`,
      "--shower-full-scale", `${scale}`);
...
function toggleMode()
{
  ...
  if (! slidemode) {
    ...
    // Save the style attribute so we can restore it when leaving slide mode.
    document.body.b6savedstyle = document.body.style.cssText;

    document.body.classList.add("full");		// Set .full on BODY
    if (useApplication)					// Hint to screenreaders
      document.body.setAttribute("role", "application");

    /* Make all children of BODY invisible. */
    for (h = document.body.firstElementChild; h; h = h.nextElementSibling)
      if (isStartOfSlide(h)) {
	h = hideSlide(h);	// Returns last element that was hidden
      } else if (h != liveregion && h != toctext)
	hideElement(h);

    /* Swap style sheets for projection and screen. */
    toggleMedia();				// Swap style sheets

    /* We don't want the browser to capture the Escape key and exit
       fullscreen mode, but only in Chrome is there a way to claim the
    ...
    // Let the screen reader announce that we are leaving slide mode.
    // TODO: This doesn't work in all browsers. The screen reader
    // seems to have other things to announce.
    speak(EXIT);

    // We no longer need to keep the browser from capturing the Escape key.
    if (navigator?.keyboard?.lock) navigator.keyboard.unlock();
    ...
    // Remove the scale and other slidemode properties.
    document.body.style.cssText = document.body.b6savedstyle;

    /* Unhide all children again, except comments if commentsVisible is false */
    for (const h of document.body.children)
      if (h !== liveregion) unhideElement(h);
    if (! commentsVisible) {
      toggleComments();
      toggleComments();
    }
    ...
    if (curslide && location.protocol !== 'about:')
      location.replace("#" + curslide.id);
  }
}
...
function toggleDarkMode(onoff)
{
  ...
  /* timeToMillisec -- convert MM:SS, DDs, DDm and DDh, etc. to milliseconds */
function timeToMillisec(s, unit)
{
  var m;

  // If no unit specified, xx:xx means minutes & seconds and xx means minutes.
  if (! unit) unit = SECONDS;

  if ((m = /^(\d+):(\d\d):(\d\d)$/.exec(s)))
    return m[1] * HOURS + m[2] * MINUTES + m[3] * SECONDS;
  if ((m = /^(\d+):(\d\d)$/.exec(s)))
    return unit === MINUTES ? m[1] * HOURS + m[2] * MINUTES :
    m[1] * MINUTES + m[2] * SECONDS;
  if ((m = /^(\d+|\d*\.\d+)s$/.exec(s))) return m[1] * SECONDS;
  if ((m = /^(\d+|\d*\.\d+)m$/.exec(s))) return m[1] * MINUTES;
  if ((m = /^(\d+|\d*\.\d+)h$/.exec(s))) return m[1] * HOURS;
  if ((m = /^(\d+|\d*\.\d+)$/.exec(s))) return m[1] * unit;
  return 0;
}
...
/* previousSlideOrElt -- previous incremental element or slide */
function previousSlideOrElt()
{
  ...
  if (incrementals.cur >= 0) {
    ...
    if (incrementalsBehavior === "forwardonly") {
      displaySlide();			// Reinitialize the current slide.
    } else {
	...
	incrementals[incrementals.cur].classList.add("active");
	speak(INCREMENTAL);
      } else {			// No preceding incremental element
	speak(SLIDE);
      }
    }
...
function gestureCancel(ev)	// eslint-disable-line no-unused-vars
{
  if (gesture.on) {
    gesture.on = false;
    /* Undo visual feedback */
...
function hashchange(ev)		// eslint-disable-line no-unused-vars
{
  var e;
...
function windowResize(ev)	// eslint-disable-line no-unused-vars
{
  if (slidemode && !switchInProgress) scaleBody(); // Recalculate the transform
}
...
function fullscreenChanged(ev)	// eslint-disable-line no-unused-vars
{
  ...
  if (switchFullscreen) {
	...
	(document.fullscreenElement && !slidemode)) {
      ...
      if (toctext?.parentElement) {toctext.close(); toctext.remove();}
      toggleMode();
    }
  }
}
...
function beforeUnload(ev)	// eslint-disable-line no-unused-vars
{
  if (secondwindow?.closed === false)
...
function addUI()
{
  ...
  if (playbuttons.length == 0) {
    ...
    playbutton.innerHTML = "<span aria-hidden=true>" + _("▶\uFE0E") +
      "</span> <span>" + _("play/<wbr>stop") + "</span>";
  ...
  if (secondwindowbuttons.length == 0) {
    ...
    secondwindowbutton.innerHTML = "<span aria-hidden=true>" + _("⧉") +
      "</span> <span>" + _("play in 2nd window") + "</span>";
  ...
  if (prevbuttons.length == 0) {
    ...
    prevbutton.innerHTML = "<span aria-hidden=true>" + _("❮") +
      "</span> <span>" + _("back") + "</span>";
  ...
  if (nextbuttons.length == 0) {
    ...
    nextbutton.innerHTML = "<span aria-hidden=true>" + _("❯") +
      "</span> <span>" + _("forward") + "</span>";
  ...
  if (hasDarkMode) {
    ...
    if (darkmodebuttons.length == 0) {
      ...
      darkmodebutton.innerHTML = "<span aria-hidden=true>" + _("◑") +
	"</span> <span>" + _("dark mode") + "</span>";
  ...
  if (commentbuttons.length == 0) {
    ...
    commentbutton.innerHTML = "<span aria-hidden=true>" + _("🗊") +
      "</span> <span>" + _("notes") + "</span>";
  ...
  if (helpbuttons.length == 0) {
    ...
    helpbutton.innerHTML = "<span aria-hidden=true>" + _("?") +
      "</span> <span>" + _("help") + "</span>";
...
function checkIfFramed()
{
  ...
  if (window.parent != window) { // Only if we're not the top document
    ...
    // Set a class to allow the style to do things, but only if we
    // have a URL. Otherwise we are probably a temporary document
...
function showURL()
{
  ...
  if (h) {
    ...
    requestAnimationFrame(() => {
      ...
      setStyle(hoverOverlay, "top", "0", "left", "0");
      ...
      const w1 = hoverOverlay.offsetWidth;
      const h1 = hoverOverlay.offsetHeight;
      const w2 = visualViewport.width;
      const h2 = visualViewport.height;
      setStyle(hoverOverlay,
	"left", Math.max(0, Math.min(w2 - w1, mouseX - 2)) + "px",
	"top", Math.max(0, Math.min(h2 - h1, mouseY - 2)) + 'px',
	"margin", "0");
    });
  }
}
...
function toggleComments()
{
  ...
  if (! commentsVisible && (fragment = location.hash.substring(1)) &&
      (e = document.getElementById(fragment)?.closest('.comment')))
    unhideElement(e);
...
{
  // Find the live region if one is defined in the slide deck.
  liveregion = document.querySelector("[role=region][aria-live=assertive]");

  // Find or set the text to speak when leaving slide mode.
  if (liveregion) savedContent = [cloneNodeClean(liveregion)];
  else savedContent = [new Text(_("Stopped."))];

  // We don't need a live region if (the non-standard) ariaNotify() exists.
  if (document.ariaNotify) {

    if (liveregion) liveregion.style.display = "none";
    ...
    // Give it a label if it doesn't have one yet.
    if (! liveregion.hasAttribute("aria-label"))
      liveregion.setAttribute("aria-label", _("Current content (live region)"));

    // Hide the live region visually, but make it available to screen readers.
    setStyle(liveregion, "position", "absolute", "top", "0", "left", "0",
      "width", "0", "height", "0", "clip-path", "rect(0 0 0 0)");
  }
}
...
function initialize()
{
  ...
  if (interactive) {		// Only add event listeners if not static
    document.body.tabIndex = 0;	// Make sure you can always tab out of a form
    ...
    //document.addEventListener("focusin", e => console.log(e.target));
  }
...
{
  // We do it here to try and make the slides look good in index mode
  // and to try and save time in slide mode. We still need to do it
  // again for each slide when it is displayed in slide mode, because
  // slides may not have a fixed height in index mode. If they do,
...
else window.addEventListener('load', initializeLayout, {once: true});
